...
const handleSignInSession = async (user, role) => {
  ...
  return { accessToken: session.token, role }
}
...
const handleSignIn = async (user, res) => {
  ...
  // Check administator
  const administator = await user.getAdministrator({ attributes: ['role'] })
  if (!administator) {
    return res.status(403).json(response({ error: 'Your account is not an administrator' }))
  }

  const role = administator.role
  const tokenSession = await handleSignInSession(user, role)
